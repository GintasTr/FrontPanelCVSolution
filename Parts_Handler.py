...
class Parts_Handler:
    ...
    # Function to determine whether barcode was correctly entered
    # and which parts are required for specific barcode
    def parts_required(self, parameters_entered):
        # This is where the front panel appearance addition/modification should occur!
        panel_17444646 = {'button': 'clock',
                          'temperature': 'celsius',
                          'controller': 'squares'}
        panel_17444648 = {'button': 'clock',
                          'temperature': 'fahrenheit',
                          'controller': 'squares'}
        panel_17444664 = {'button': 'trapeze',
                          'temperature': 'fahrenheit',
                          'controller': 'figures'}
        panel_17444661 = {'button': 'trapeze',
                          'temperature': 'celsius',
                          'controller': 'figures'}

        # Check if any parameters were entered
        if len(parameters_entered) < 2:
            # Return the error if no parameters were entered
            return "ERROR - barcode not entered as a parameter."
        ...
        if parameters_entered[1] == "17444646-01":
            required_panel = panel_17444646
        elif parameters_entered[1] == "17444648-01":
            required_panel = panel_17444648
        elif parameters_entered[1] == "17444664-01":
            required_panel = panel_17444664
        elif parameters_entered[1] == "17444661-01":
            required_panel = panel_17444661
        else:
            return "ERROR - incorrect barcode enetered"
